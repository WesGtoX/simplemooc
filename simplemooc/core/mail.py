...
def send_mail_template(
	...
	context,	# o contexto que vai servir para esse template.
	...
	fail_silently=False		# falha silenciosa que é uma variável que precisa no 'email.send' para saber quando o envio de e-mail falhar, ele vai lançar ou não essa ação.
):

	message_html = render_to_string(template_name, context)		# vai ser a renderizaçãodo do template no contexto que foi mandado.
	...
	email = EmailMultiAlternatives(		# 'EmailMultiAlternatives' é uma classe do pacote 'django.core.mail', que é um e-mail que tem várias alternativas. tem um conteúdo principal e cria vários alternativos.
		...
		to=recipient_list	# para quem vai.
	)
	email.attach_alternative(message_html, "text/html")	# adiciona uma alternativa, eu vou dizer qual o conteúdo alternativo, o texto 'html'.
