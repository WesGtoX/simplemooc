...
class Thread(models.Model):		# justamente o tópico do fórum.
	
	title = models.CharField('Título', max_length=100)		# título do tópico.
	slug = models.SlugField('Identificador', max_length=100, unique=True)		# 'SlugField' por padrão cria com um 'index' no banco de dados para indexar. A tendência é que seja único, mas ele não especifica que tem que ser.
...
class Reply(models.Model):
	...
	class Meta:
		...
		ordering = ['-correct', 'created']		# o valor booleano 'True' é maior que o 'False', então tem que ser ordenado de forma 'decrescente'. E os outos ordenam pela data.
...
def post_save_reply(created, instance, **kwargs):
	...
	if instance.correct:		# toda vez que salvo a resposta, ele verifica se a resposta é uma resposta correta.
		instance.thread.replies.exclude(pk=instance.pk).update(		# ele pega a instância, que é a resposta, pega a 'thread' em questão, busca todas as resposta...
...
#def post_deleate_reply(instance, **kwargs):		# porque, toda vez que uma instância for removida, uma reposta for removida, a ideia é que faça essa mesma lógica, só que ao contrário.
#	instance.thread.answers = instance.thread.answers - 1
#	instance.thread.save()
