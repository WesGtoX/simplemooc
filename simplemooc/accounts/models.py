...
class User(AbstractBaseUser, PermissionsMixin):
	...
	email = models.EmailField('E-mail', unique=True)	# e-mail que nesse caso único, é a grande diferença para o do Django.
	...
	date_joined = models.DateTimeField('Data de Entrada', auto_now_add=True)	# essa data é importante na criação do superuser, é uma recomendação par aser compativel com a app Django.

	objects = UserManager()		# o manger que tem algumas coisas úteis para nós.

	USERNAME_FIELD = 'username'		# para ser compátivel com alguns comando e algumas outras coisas do Django, app de usuário. Indica o campo que é único e vai ser a referência na hora do login. Por padrão é 'login', poderia ser o 'email'.
	REQUIRED_FIELDS = ['email']		# utilizado no comando de criação 'superusuario', vai utilizar esse compando porque vai precisar apagar o banco de dados, e vamos criar outro usuário atráves desse comando.

	def __str__(self):		# esse methodo é a representação do skin do usuário
		return self.name or self.username		# se tiver 'name', retorna o nome, caso contrário retorna o 'username'

	def get_short_name(self):	# a app de usuário de Django também pega as implementações de alguns methodos, não é obrigatório, mas é interessante para funcionamento do 'admin'.
