...
@login_required		# essa função é chamada passando como primeiro parâmetro, a função que vai ser executada, verifica se o usuário está logado, se estiver ok, se não estiver ele faz um 'redirect' para a página de 'login', colocando um parêmetro na 'url' o 'next'.
...
def register(request):
	...
	if request.method == 'POST':	# verifica se o método é 'post'...
		form = RegisterForm(request.POST)	# ...se for 'post', vou pegar o formulário...
		if form.is_valid():
...
def password_reset(request):
	...
	context = {}
	form = PasswordResetForm(request.POST or None)		# forma mais prática para que os dados não sejam validados, quando for 'POST' de fato ele vai validar os dados, e quando não for, não vai ser validado.
	if form.is_valid():
		form.save()
...
def edit(request):
	...
	context = {}	# dicionário.
	if request.method == 'POST':	# verifica se o método é 'POST'.
		form = EditAccountForm(request.POST, instance=request.user)		# se for 'POST' chama o 'EditAccountForm', passando o 'POST' que são os dados. 'instance' é a instância que está sendo alterada, o 'model', que é o 'request.user', usuário atual da sessão.
