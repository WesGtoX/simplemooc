...
def index(request):
	courses = Course.objects.all()		# retorna os objetos cadastrados no banco de dados.
	template_name = 'courses/index.html'
	context = {		# dicionário passado no final.
		'courses': courses
	}
...
def details(request, slug):
	...
	context = {}
	if request.method == 'POST':	# verifica se o método é um 'post' ou se é um método 'get'.
	...
	context['form'] = form		# variável form.
...
def enrollment(request, slug):
	...
	enrollment, created = Enrollment.objects.get_or_create(		# esse método vai ser passado um filtro, qual vai ser o 'user' do 'request. Ele retorna uma 'tupla', a inscrição se ouver, caso não, será criado, e um boleano dizendo se criou ou não.
...
@login_required
def undo_enrollment(request, slug):		# todas as páginas associadas ao curso está com 'slug', mas não necessariamente é preciso. Não tem a necessidade de ser uma 'url' descritiva, poderia ser coloca o 'id' da matrícula diretamente. Porém estamos deixando padrão com o 'slug' em todas.
...
def announcements(request, slug):
	...
	context = {
		'course': course,
		'announcements': course.announcements.all()	# todos os anúncios do curso estão nessa variável.
	}
	return render(request, template, context)
...
def material(request, slug, pk):
	course = request.course
	material = get_object_or_404(Material, pk=pk, lesson__course=course)	# não tem relacionamento com o curso, mas tem relacionamento com a aula que tem relacionamento com o curso. Quando coloca dois underscore '__' vou acessar uma propriedade já do outro objeto 'lesson__course'.
