...
class ContactCourseTestCase(TestCase):		# toda vez que inicializar a classe 'ContactCourseTestCase', eu executo o 'setUpClass', e para cada teste que vou executar, eu executo o 'setUp' e o 'tearDown'. depois que terminar todos os testes, eu vou executar o 'tearDownClass'.
	...
	def test_contact_form_error(self):		# formulário de teste criado quando ouver erro. Vou submeter o formulário e que quero que ele de erro. Para isso tem um atalho em Django que é o 'assertFormError'.
	...
	def test_contact_form_success(self):	# a ideia é funcionar corretamente. e o 'post' vai ser enviado.
		...
		self.assertEqual(len(mail.outbox), 1)	# no Django tem como testar se o e-mail foi enviado. Com o 'assertEqual' que pega o 'mail.outbox', faz o len() dele que tem que ser igual a 1 email. No ambiente de testes, os e-mail ficam no 'outbox'.
		self.assertEqual(mail.outbox[0].to, [settings.CONTACT_EMAIL])	# teste para quem for o email. Se foi para 'settings.CONTACT_EMAIL' que é uma lista.
